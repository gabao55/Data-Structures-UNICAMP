...
int main() {
    ...
    while (keep_running) {
        ...
        if (strcmp(cmd, "info") == 0) {
            keep_running = nodes_number(T->root);
            printf("nos: %d, ", keep_running);
            keep_running = leaves_number(T->root);
            printf("folhas: %d, ", keep_running);
            keep_running = height(T->root) - 1;
            if (keep_running < 0)
                keep_running = 0;
            printf("altura: %d\n", keep_running);
            keep_running = 1;
        ...
        if (strcmp(cmd, "caminho-mais-longo") == 0) {
            if (!T->root)
                printf("arvore vazia\n");
            else {
